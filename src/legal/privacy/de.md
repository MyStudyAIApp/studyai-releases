...
Wir erheben automatisch Daten von Ihnen, wenn Sie z. B. mystudyai.eu besuchen.
- Nutzungsdaten
- Tracker
- IP-Adresse
...
DeepInfra ist ein KI-Inferenzdienst, der für drei konkrete Aufgaben eingesetzt wird: die vom Nutzer in dieser Anwendung erstellten Audioaufnahmen in Text umzuwandeln (Whisper-Modell) und die Fotos zu lesen, die der Nutzer über die Funktionen „Notizen scannen“, „Mein Heft“ und „Aufgabe lösen“ einscannt. Die Audiodaten bzw. das Bild werden an den Dienst übermittelt, der den transkribierten Text und im Fall von „Aufgabe lösen“ auch die schrittweise Lösung zurückgibt. Das Foto wird vom Inferenzdienst nicht aufbewahrt: er gibt ausschließlich den Text zurück. Die Fotos, die der Nutzer mit dem Scanner der Anwendung aufnimmt, werden bei keiner dieser drei Funktionen auf unseren Servern gespeichert: Aufbewahrt wird ausschließlich der resultierende Text. In den mobilen Anwendungen wird das Foto vorübergehend auf dem Gerät des Nutzers selbst gespeichert, solange der Scan läuft, damit er bei einem Verbindungsfehler nicht verloren geht; es wird vom Gerät gelöscht, sobald der Scan gespeichert wurde. Schlägt der Upload fehl, bleibt es für einen erneuten Versuch erhalten, der Nutzer kann es auf demselben Bildschirm verwerfen und es wird in jedem Fall nach 7 Tagen automatisch gelöscht. Weder die Audiodaten noch die Bilder noch die Transkriptionen werden zur Profilerstellung oder zu Werbezwecken verwendet. Der Dienst wird nur in der Webversion und in den mobilen Anwendungen eingesetzt: die Desktop-Anwendung transkribiert Audio lokal auf dem Gerät, ohne Daten zu übermitteln. Die dritte Aufgabe ist die Beantwortung der Fragen, die der Nutzer im Hilfe-Chat der Eule eingibt (siehe „Hilfe-Chat der Eule“); ist DeepInfra nicht verfügbar, erzeugt Mistral AI diese Antwort.
...
### Link, über den du kommst
Wenn du über einen markierten Link (zum Beispiel `mystudyai.eu/?de=tiktok`) oder von einer anderen Website auf unsere Website kommst, speichert dein Browser diese Markierung und den Namen dieser Website im lokalen Speicher deines Geräts (kein Drittanbieter-Cookie, wird mit niemandem geteilt). Wenn du dich registrierst, speichern wir sie zu deinem Konto und löschen sie vom Gerät; gespeichert wird sie nur bei neu erstellten Konten. Sie dient nicht dazu, dich zu verfolgen, und nicht der Werbung: nur um zu zählen, über welche Kanäle Registrierungen kommen. Rechtsgrundlage: unser berechtigtes Interesse, die Wirksamkeit unserer Bekanntmachung zu kennen. Die Angabe wird mit deinem Konto gelöscht.
