...
We automatically collect data from you for example when you visit  mystudyai.eu.
- Usage Data
- Trackers
- IP address
- other type of data
...
### Resend
Service for sending account and service emails (account confirmation, password reset, notices) and the welcome emails described in “Study reminders and welcome emails”. Sent from the EU (eu-west-1), with account metadata and logs hosted in the USA.

### Amazon SES (AWS)
Indirect sub-processor of Resend: emails are sent through Amazon SES (eu-west-1, Ireland) behind the scenes.

### ImprovMX
Forwarding service for incoming mail to the mystudyai.eu domain (support@mystudyai.eu mailbox).
...
DeepInfra is an AI inference service used for three specific tasks: transcribing into text the audio recordings made by the User within this Application (Whisper model), and reading the photographs the User scans through the “Scan notes”, “My notebook” and “Solve exercise” features. The audio or the image is sent to the service, which returns the transcribed text and, in the case of “Solve exercise”, the step-by-step solution as well. The photograph is not retained by the inference service: it only returns the text. Photographs taken by the User with the Application's scanner, in any of those three features, are not stored on our servers either: only the resulting text is kept. In the mobile applications the photograph is stored temporarily on the User's own device while the scan is in progress, so that it is not lost if the connection fails, and it is deleted from the device as soon as the scan has been saved; if the upload fails it is kept so that it can be retried, the User may discard it from that same screen and, in any event, it is deleted automatically after 7 days. Neither the audio, nor the images, nor the transcriptions are used for profiling or advertising purposes. It is used only in the web version and the mobile applications: the desktop application transcribes audio on the device itself, without sending anything. The third task is answering the questions the User types into the owl help chat (see “Owl help chat”); if DeepInfra is unavailable, that answer is generated by Mistral AI.
...
### Sign in with Google
Identification service provided by Google Ireland Limited (Ireland) that lets the User create and access their account using their Google account, without having to choose a new password. It is an optional alternative to signing up with an email address and password. When it is used, this Application receives from Google the basic profile data needed to create the account, and Google learns that the User has accessed this Service. This Application never receives the password of the User's Google account.

### Personal Data processed:
- Email address
- Account name
- Google account identifier
- Profile picture, where the User has one

### Service provided by:
- Google Ireland Limited (Ireland)

### Notifications
This Application may send alerts to the User's device: exam reminders, advance warnings before their files are automatically deleted, notice that a scan the User left running in the background has finished, and the study reminders described in “Study reminders and welcome emails”. In the mobile applications these alerts are scheduled and shown by the application itself on the device: they do not pass through our servers or through the notification services described below. Alerts are only shown if the User grants their device's notification permission or expressly enables them from within the Application. The User may disable them at any time, one by one from the Application's settings or all at once from those of their browser or operating system; for web alerts, doing so deletes the subscription.

### Web notifications (Web Push)
When the User enables alerts, their browser generates a delivery address (endpoint) and encryption keys which this Application stores in order to deliver the message. Actual delivery is carried out by the notification service of the browser the User uses — Google (Chrome and derived browsers), Mozilla (Firefox) or Apple (Safari) — which by its very nature knows when delivery takes place. The content of the alert travels end-to-end encrypted and those services cannot read it.

### Personal Data processed:
- Delivery address (endpoint) generated by the browser
...
### RevenueCat
Purchase and subscription management service (RevenueCat, Inc., USA), used to verify active subscriptions, manage the plan catalog, and sync subscription status across platforms (Web, Android, desktop).
...
The original file (PDF or image) that the User uploads from their device is retained for 10 days from the upload date and is then automatically deleted from our storage. Photographs taken with the Application's own scanner are not retained at any point: only the resulting text is kept. Content generated from that file (summaries, study cards, exams, outlines, and other generated material) is not affected and is retained without a time limit, until the user deletes it or closes their account. An email notice is sent 3 days before the original file is deleted; the user can disable this notice from within the application, and the date they disabled it is recorded.
...
### Study reminders and welcome emails
To help you get the most out of the service you already use, MyStudy may:
- **Show alerts on your phone** (mobile applications only): an upcoming exam you added, a practice exam you left unfinished, a subject with flashcards due for review and, if you have not opened the application for 7 days, a single reminder. At most one alert per day and two per week, never between 21:00 and 09:00. They are generated on your own device from your study data: nothing is sent to third parties.
- **Send you at most two welcome emails**, the day after you sign up and after 7 days, only if you have not started using the application yet, with tips to get started. After that we will not send you any more emails of this kind. They are sent through Resend (see “Communication”).
- **Occasionally send you emails with MyStudy news** (new features or important improvements to the service), at most one a month. Only about MyStudy, never about third-party products. Legal basis: our legitimate interest and art. 21.2 of the Spanish LSSI (communications about the service itself to existing users). Every email has a link to unsubscribe in one click, which also unsubscribes you from the welcome emails.

We do not use pixels or trackers in emails: we do not know whether you open them. Legal basis: for phone alerts, your consent, which you give by allowing notifications and can withdraw at any time; for the emails, our legitimate interest in you making good use of the service you signed up for, under Article 21.2 of Spanish Law 34/2002 (LSSI), which allows writing to existing users about the same service. You can object at any time, free of charge: every email includes a one-click unsubscribe link, and in the application settings you can turn off the emails and each type of alert separately. This does not affect essential service emails (account confirmation, password change, deletion or payment notices).
...
### Minors
The Service is not aimed at children under 16. The Terms of Use require that minimum age to create an account or, where the User is below it, the express authorisation of their parents or legal guardians, which the User declares to hold upon registering. No data is knowingly collected from anyone below that age. Should the Owner become aware that an account was created in breach of this requirement, it will be deleted along with its data. Parents or guardians who identify such a situation may report it to support@mystudyai.eu and it will be handled without delay.
