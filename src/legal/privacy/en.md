...
### Notifications
This Application may send alerts to the User's device: exam reminders, advance warnings before their files are automatically deleted, and notice that a scan the User left running in the background has finished. This last one is generated by the application on the device itself: it does not pass through our servers or through the notification services described below. Alerts are optional and are only sent if the User expressly enables them from within the Application. The User may disable them at any time from the Application's settings or from those of their browser or operating system, and doing so deletes the subscription.
