...
export default function RetentionChip({ createdAt, retentionDays = 10, downloadedAt, downloadedPlatform }) {
  ...
  const base = dias === 0 ? i18n.t('retention.today') : i18n.t('retention.inDays', { count: dias })
  const text = downloadedAt ? `${base} · ${i18n.t('retention.downloadedOn', { where: platformLabel(downloadedPlatform) })}` : base
